...
from .models import MajorInfo, UserType, UserInfo, ClassInfo, Attendence, Notice, Leave
# django自带加密解密库
...
def change_pwd(request):
    if request.method == 'GET':
        ...
        if request.POST.get('new_email',default=False):

            global change_pwd_email
            change_pwd_email = request.POST.get('new_email')
            from ..static.py import send_email
            from random import randint
            code = 'abcdefghijklmnopqrstuvwxyz0123456789'
            global captcha
            captcha = code[randint(0, 35)]+code[randint(0, 35)]+code[randint(0, 35)]+code[randint(0, 35)]
            send_email.send_email(change_pwd_email, captcha)
        elif request.POST.get('captcha', default=False):

            change_pwd_captcha = request.POST.get('captcha')

            if change_pwd_captcha != captcha:
                ...
                print('验证码正确')
                if request.POST.get('new_pwd', default=False):

                    new_pwd = request.POST.get('new_pwd')
                    hl = hashlib.sha1()
                    hl.update(new_pwd.encode('utf8'))
                    new_pwd = hl.hexdigest()
                    change_obj = UserInfo.objects.get(email=change_pwd_email)
                    if change_obj:
                        change_obj.password = new_pwd
                        change_obj.save()
                        print('修改密码成功!')
        ...
        if rank.user_type.caption == 'admin':
            ...
            if request.method == 'POST':
                ...
                if not MajorInfo.objects.filter(name=add_major_name):
                    MajorInfo.objects.create(name=add_major_name)
...
#                     Exam.objects.create(point=point,content_id=con
# #         return render(request, 'exam_manage_denied.html')tent_id,user_id=stuID,detail=detail)
#                 # print(request.body)
